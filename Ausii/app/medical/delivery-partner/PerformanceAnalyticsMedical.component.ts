...
@Component({
  standalone: true,
  selector: 'app-performanceanalyticsmedical',
  imports: [CommonModule],
  template: `
    <div class="min-h-screen bg-[#f1f5f9] text-[#1e293b] font-sans overflow-x-hidden flex flex-col">
      <nav class="bg-white/80 backdrop-blur-xl sticky top-0 z-50 px-6 py-5 border-b border-slate-200 flex justify-between items-center">
        <div class="flex items-center gap-4">
           <div class="w-10 h-10 bg-indigo-600 rounded-3xl flex items-center justify-center rotate-6 shadow-xl shadow-indigo-600/20">
              <span class="text-white font-black text-xl italic select-none">S</span>
           </div>
           <div>
              <h2 class="text-xs font-black tracking-widest text-indigo-400 uppercase select-none tracking-tighter">SUPERAPP LOGISTICS</h2>
              <p class="text-[10px] font-bold text-slate-400 opacity-60">MISSION READY CORE v4.1</p>
           </div>
        </div>
        <div class="flex items-center gap-2 bg-green-50 px-4 py-2 rounded-full border border-green-100 transform hover:scale-105 transition-all cursor-pointer">
           <div class="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_10px_#22c55e]"></div>
           <span class="text-[10px] font-black tracking-widest text-green-600">LIVE MISSION SYNC</span>
        </div>
      </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div class="lg:col-span-8 flex flex-col gap-10">
            <header class="max-w-3xl">
              <span class="inline-block px-3 py-1 bg-indigo-100 text-indigo-600 rounded-lg text-[10px] font-black tracking-widest uppercase mb-4 shadow-sm border border-indigo-200 animate-pulse">#427 | Performance Analytics Medical Module</span>
              <h1 class="text-5xl md:text-7xl lg:text-8xl font-black text-slate-900 leading-[1.1] tracking-tighter mb-6 group cursor-default">
                Performance Analytics Medical
              </h1>
              <p class="text-lg md:text-xl text-slate-500 font-medium leading-relaxed opacity-80 max-w-2xl">
                Ready for high-velocity dispatch and real-time execution. Optimized for mobile precision and desktop visualization. Responsive design is fully active for all platforms.
              </p>
            </header>
            ...
            <section class="relative bg-white rounded-[3rem] p-10 md:p-16 shadow-2xl shadow-indigo-600/5 group hover:shadow-indigo-600/10 transition-all border border-slate-100/50 min-h-[450px] md:min-h-[600px] flex items-center justify-center overflow-hidden">
               ...
               <div class="relative z-10 text-center space-y-8 animate-fade-in scale-up">
                  <div class="w-32 h-32 md:w-48 md:h-48 bg-slate-50/80 rounded-[3rem] mx-auto flex items-center justify-center border-2 border-dashed border-indigo-200 group-hover:border-indigo-500/40 group-hover:bg-white transition-all transform group-hover:scale-110 shadow-sm relative overflow-hidden">
                  ...
                  <p class="text-slate-300 group-hover:text-slate-500 transition-colors font-black text-2xl md:text-3xl lg:text-4xl italic tracking-widest uppercase leading-tight select-none">
                    [ Performance Analytics Medical Interface Base ]
                  </p>
          ...
          <aside class="lg:col-span-4 space-y-10">
             ...
             <div class="bg-[#1e1b4b] p-10 md:p-12 rounded-[3.5rem] text-white shadow-2xl shadow-indigo-900/40 relative overflow-hidden group hover:-translate-y-1 transition-all duration-500 cursor-pointer">
                ...
                <h3 class="text-2xl font-black mb-6 tracking-tight uppercase italic text-indigo-100 select-none">Mission Hub</h3>
                <p class="text-indigo-100/60 font-medium leading-relaxed mb-10 text-base italic group-hover:text-indigo-100/90 transition-colors">
                  System ID #427 is active. Priority dispatch rules are now optimized for Performance Analytics Medical mission.
                </p>
                <button class="w-full py-6 bg-indigo-600 text-white rounded-[2rem] font-black text-sm uppercase tracking-[0.2em] shadow-xl shadow-indigo-600/20 hover:bg-white hover:text-indigo-900 transition-all active:scale-95 border border-indigo-500/0 hover:border-indigo-500">
                  EXECUTE MISSION
                </button>
             </div>
             ...
             <div class="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-xl shadow-slate-200/20 hover:shadow-indigo-600/5 transition-all">
                <h4 class="text-slate-300 font-black mb-8 uppercase text-[10px] tracking-[0.4em] italic select-none">Telemetry Streams</h4>
                <div class="space-y-6">
                   <div class="flex justify-between items-center mb-1">
                      <span class="text-xs font-black text-slate-400 tracking-wider">LATENCY</span>
                      <span class="text-xs font-black text-indigo-600 uppercase tracking-tighter italic">42ms PRO-SYNC</span>
                   </div>
                   ...
                   <div class="flex justify-between items-center mb-1 mt-4">
                      ...
                      <span class="text-xs font-black text-green-500 uppercase tracking-tighter italic">100% SECURE</span>
                   </div>
                   <div class="h-3 w-full bg-slate-50 rounded-full overflow-hidden border border-slate-100 p-[2px]">
      ...
      <footer class="mt-auto px-10 py-12 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
        <p class="text-slate-400 font-bold text-[10px] tracking-[0.5em] uppercase italic opacity-50 select-none tracking-tighter">SUPER LOGISTICS ARCHITECTURE | ANGULAR CORE</p>
  ...
  styles: [`
    ...
    .scale-up { animation: scaleUp 1s ease-out; }
    @keyframes scaleUp { from { transform: scale(0.9); opacity: 0; } to { transform: scale(1); opacity: 1; } }
  `]
})
export class PerformanceAnalyticsMedicalComponent implements OnInit {
  constructor() {}
  ngOnInit(): void {}
}
